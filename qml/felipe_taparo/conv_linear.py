...
class QCNNLinear(nn.Module):
    ...
    def fit(self, train_loader, test_loader, epochs=50, lr=0.01, device='cpu', verbose=True):
        ...
        for epoch in range(epochs):
            ...
            if verbose:
                append_log('./qcnn_linear.txt', f"""Epoch {epoch+1}
                                          Time: {time() - init_time:.2f}s
                                          Train: loss: {avg_train_loss:.4f}, acc: {train_acc:.4f}
                                          Test: loss: {test_loss:.4f}, acc: {test_acc:.4f}\n""")

    def save_weights(self, filename='qcnn_linear_weights.pth'):
...
def main():
    ...
    model = QCNNLinear(circuit=circuit).to(device)
    model.fit(
    ...
    model.save_weights(f'artifacts/qcnn_linear_last.pth')
