...
def data_load_and_process(dataset, classes=[0, 1], feature_reduction='resize256', binary=True):
    """
    Carrega, filtra, normaliza e reduz a dimensionalidade dos datasets MNIST ou Fashion MNIST 
    para treinamento de classificadores quânticos (como QCNN ou Classificadores Hierárquicos).

    A função converte o problema original de 10 classes para um problema de classificação 
    binária, mapeando as labels de acordo com o argumento `classes` e `binary`, e extrai 
    características da imagem (redução de dimensionalidade) para adequar ao número de qubits.

    Args:
        dataset (str): Nome do dataset a ser carregado. Opções suportadas: 'mnist', 'fashion_mnist'.
        classes (list de int ou str, opcional): Define como as classes serão agrupadas/filtradas 
            para a classificação binária. 
            - Se list (ex: [0, 1]): Filtra o dataset para manter apenas as imagens destas duas classes.
            - Se 'odd_even': Agrupa as classes em ímpares (1) e pares (0 ou -1).
            - Se '>4': Agrupa as classes em maiores que 4 (1) e menores ou iguais a 4 (0 ou -1).
            Padrão é [0, 1].
        feature_reduction (str, opcional): Método de redução de dimensionalidade a ser aplicado.
            - 'resize256': Redimensiona a imagem e achata para 256 features.
            - 'pcaX' (onde X é 8, 12, 16, 30, 32): Aplica PCA para reduzir a X features.
            - 'autoencoderX' (onde X é 8, 12, 16, 30, 32): Treina um Autoencoder denso e usa 
              o encoder para extrair X features latentes.
            *Nota: Algumas opções de PCA e Autoencoder realizam um reescalonamento dos dados 
            para o intervalo [0, π], o que é essencial para 'Angle Embedding' em circuitos quânticos.*
            Padrão é 'resize256'.
        binary (bool, opcional): Define o mapeamento matemático das labels retornadas.
            - True: Mapeia as labels para {1, -1} (comum em otimização quântica/SVM).
            - False: Mapeia as labels para {1, 0} (comum em redes neurais clássicas/entropia cruzada).
            Padrão é True.

    Returns:
        tuple: Uma tupla contendo (X_train, X_test, Y_train, Y_test).
            - X_train (numpy.ndarray): Tensores de features de treinamento.
            - X_test (numpy.ndarray): Tensores de features de teste.
            - Y_train (list de int): Labels de treinamento mapeadas.
            - Y_test (list de int): Labels de teste mapeadas.
    """
